...
void peba(hitbox* h, mouse_game* m, JogoDados* Informacao, Fase fases[], int fps, int delay, ALLEGRO_BITMAP* img);
...
void mostrar_tela_final(int w, int h, ALLEGRO_FONT* fonte);
...
int main() {
    ...
    int delay_anim = 3;
    hitbox inimigos[10];
    JogoDados info_jogo = { 10, 0, 0, 0, 0, 0, 0, 0, false, 0, 0, 0, 0, 0 };
    Fase fases[10];
                ...
                if (estado_atual == MENU) {
                    for(int i=0; i<qtd_btns_menu; i++) {
                        if (btns_menu[i]->mouse_hover) {
                            ...
                            else if(strcmp(btns_menu[i]->id, "CREDITOS") == 0) estado_atual = CREDITOS;
                            else if(strcmp(btns_menu[i]->id, "SAIR") == 0) rodando = 0;
                        }
                    }
                }
                    ...
                    if (info_jogo.pause && !info_jogo.fim_fases && !info_jogo.transicao) {
                        ...
                        if (btn_voltar->mouse_hover) { info_jogo.pause = false; estado_atual = MENU; }
        ...
        if (desenhar && al_is_event_queue_empty(fila_eventos)) {
            ...
            if (estado_atual == MENU) {
                ...
                if (info_jogo.fim_fases) {
                    ...
                    mostrar_tela_final(LARGURA_TELA, ALTURA_TELA, assets.texto);
                }
                ...
                if (!info_jogo.fim_fases) {
                    for (int cont = 0; cont < fases[info_jogo.fase_atual].quantidade_pebas; cont++) {
                        peba(&inimigos[cont], &mouse_logic, &info_jogo, fases, fps, delay_anim, sprite_peba);
                    }
...
void mostrar_tela_final(int w, int h, ALLEGRO_FONT* fonte) {
    ...
    al_draw_text(fonte, al_map_rgb(255, 255, 255), w/2, h/2 - 40, ALLEGRO_ALIGN_CENTRE, "FIM DAS FASES!");
    al_draw_text(fonte, al_map_rgb(255, 255, 255), w/2, h/2 + 10, ALLEGRO_ALIGN_CENTRE, "Pressione ESPACO para jogar de novo");
    al_draw_text(fonte, al_map_rgb(255, 255, 255), w/2, h/2 + 40, ALLEGRO_ALIGN_CENTRE, "Pressione BACKSPACE para MENU");
}